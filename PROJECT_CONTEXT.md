...
| `employees` | Store employee accounts | `id` (uuid), `code` (unique, e.g. `STR-1042`), `password_hash` (independent credential — see Authentication Flow), `whatsapp_number` (contact info only), `label` (display name), `sector` (nullable free text, e.g. "PRODUCTION"/"HUMAN RESOURCE"/"FINANCE AND ACCOUNTS" — added 2026-07-31, migration `0005_light_the_watcher.sql`; the employee's HR organizational unit, informational profile field only, entirely unrelated to the 7 fixed ticket-routing departments in `tickets.department_id`), `is_active` | Referenced by `tickets.employee_code`, `broadcast_recipients`, `profiles.identifier` |
...
# Required Update (DEPLOYED LIVE, 2026-07-25)

> **Status**: all 7 items are live in production as of 2026-07-25. Migration applied to the live Supabase DB, `fix/resolve-merge-conflicts` merged into `master` and pushed (Render auto-deployed the backend from it), frontend rebuilt and deployed to Cloudflare Workers. Item 3's wipe (`server/src/db/wipe-activity-data.ts`) was run against production on 2026-07-25 — all tickets/replies/broadcasts/notifications/attachments cleared, every account untouched. Item 4's consolidation script (`server/src/db/consolidate-admins.ts`) was **not** run against production — the client tested multi-department admin assignment locally instead and it superseded the fixed KN-02-gets-everything design (see "Additional Changes" below); the live `KN-02`–`KN-08` admins were left as-is rather than force-consolidated. Re-check current admin state before assuming this script still needs to run.
...
# Additional Changes (DEPLOYED LIVE, 2026-07-25)

Four more changes requested the same day, after the Required Update pack above — deployed live 2026-07-25 along with everything else:
...
- ~~Possible multi-department admin support~~ — **shipped 2026-07-25**, see Change Log.
- **Audit logs for the master admin panel** (requested 2026-07-26). A proper audit log — a chronological record of who did what, when, and often from where, distinct from a normal application log because it specifically tracks actions that matter for accountability. Examples: a user signing in/out, a record being created/edited/deleted, permissions being changed, an admin changing settings. Used for security investigations, compliance, troubleshooting, and accountability. Note this is a step beyond the existing `notifications` table (which is a delivery/UI-feed log, not an accountability log) — a real audit log would need its own dedicated table and to hook into every mutating action across the app (account CRUD, ticket status changes, department reassignment, password resets, etc.), not just the ticket/broadcast events `notifications` already covers.
- **Internal chat between employees, department admins, and the master admin** (requested 2026-07-26) — explicitly framed by the client as replacing WhatsApp for professionalism. Distinct from the existing ticket reply thread and from Alerts/broadcasts: this would be genuine chat (department-employee-to-department-employee, employee-to-their-admin, admin-to-master-admin), not tied to a specific ticket. Real-time delivery would reuse the existing Socket.IO infrastructure, but this is a materially new feature (its own data model, its own UI surface) — not a small addition, needs real scoping before implementation (who can message whom, is it 1:1 or department-wide group chat, message history/retention, etc.).
- **No rate limiting anywhere in the backend** (flagged 2026-07-30). `server/src/app.ts` only applies `cors` + `express.json()` — no `express-rate-limit` or any custom throttling exists. Biggest concrete exposure: `/v1/auth/employee/login` and `/v1/auth/admin/login` accept unlimited attempts, so there's no brute-force/lockout protection on passwords. Deliberately deferred, not urgent — revisit when there's time (e.g. `express-rate-limit` with a tight limit on the two login routes and a looser global limit elsewhere).
...
- **Feature: richer employee bulk import (code + name + department)** (2026-07-31). Previously "Import from spreadsheet" only ever extracted an employee code column, ignoring everything else in the file. Added `employees.department_id` (nullable FK to `departments`, `onDelete: set null`, migration `0004_perpetual_sharon_ventura.sql`) — the employee's home department, a profile field only, with no effect on ticket routing (employees still pick a department per-ticket as before). `server/src/utils/spreadsheet.ts` rewritten (`extractEmployeeCodes` → `extractEmployeeRows`) to independently locate Code/Name/Department columns by header (case-insensitive, order-independent — extra columns like "Sr. No." are ignored). Department text is matched case-insensitively against `departments.name` or `.slug`; an unmatched department string still creates the employee (department left blank) and is surfaced in the import result as `unmatchedDepartment`. Manual "New employee"/"Edit" form in Manage Credentials also gained a Department dropdown, and the credentials table gained a Department column (WhatsApp column removed from the table, Label renamed to Name throughout the UI — see below). Verified end-to-end against the local `kinconnect_test` DB (exact-name match, slug/case-insensitive match, and unmatched-department fallback all confirmed correct via direct DB inspection + UI).
- **Fix: department column detection was too strict, plus re-import now backfills** (2026-07-31, same day). First real-world use of the feature above (a 543-row production import) created every employee with the right name but **no department** — the department header in the user's file didn't exactly match the fixed variant list (`"department"`/`"dept"`/`"department name"`). Column detection rewritten in `spreadsheet.ts` to substring-match (`includes("dept")`, `includes("name")`) instead of requiring an exact string, so phrasings like "Dept Name" now work; department is resolved before name so "Department Name" isn't double-claimed. Since bulk-import previously only ever *created* new rows and silently skipped existing codes, re-uploading the same file after this fix wouldn't have fixed the already-imported 543 employees — `bulkCreateEmployees` now also backfills `label`/`department_id` for existing employees when the sheet provides a value and the existing row's field is currently blank (never overwrites an already-set value). New `updatedExisting` field on the import result surfaces this. Verified locally: a pre-existing employee with no department got backfilled on re-import; a new employee in the same file was created fresh with department in one pass.
- **UI change: WhatsApp column removed, Label renamed to Name** (2026-07-31, same request). The Manage Credentials table no longer shows a "WhatsApp (Password)" column (redundant now that most employees are bulk-imported with a shared default password, not a per-employee WhatsApp number) and its "Label" column/form field is now labeled "Name" everywhere in the UI (the underlying `employees.label` DB column and API field name are unchanged — this was a display-text-only change). The WhatsApp number field remains in the "New employee" form itself, since manually-created employees still use it as their initial password.
- **Correction: "Department" → "Sector" is a different field, not a rename** (2026-07-31, same day). The user's actual employee spreadsheet was inspected directly (`updated employees data sheet csv file.csv`, project root) and its "SECTOR" column contains 21 HR organizational-unit values (STORE OPERATIONS, PRODUCTION, SUPPLY CHAIN, PROCUREMENT, FINANCE AND ACCOUNTS, HUMAN RESOURCE, INFORMATION TECHNOLOGY, etc.) — entirely unrelated to the 7 fixed ticket-routing departments (Infrastructure, Hospitality Services, etc.). Reusing the `departments` table/FK for this (the original 2026-07-31 design) was wrong. **Migration `0005_light_the_watcher.sql`** drops `employees.department_id` (and its FK) and adds `employees.sector` (plain nullable text — no dictionary table, no name/slug resolution, no `unmatchedDepartment` concept; whatever text is in the sheet's Sector/Dept column is stored as-is). Bulk import, manual create/edit, and the Manage Credentials table were all updated accordingly (Sector is a free-text input, not a dropdown). Verified against the user's real 543-row file locally: all 543 employees backfilled with their correct sector in one pass (STORE OPERATIONS ×229, PRODUCTION ×89, SUPPLY CHAIN ×59, ... 21 distinct values, exact counts confirmed via direct DB query).
- **Destructive: production employees + non-master admins wiped** (2026-07-31). Per explicit user request, ahead of the richer bulk-import feature above: `server/src/db/wipe-employees-and-subadmins.ts` deletes all `employees` and all non-master `admins` (master admin(s), `is_master = true`, explicitly preserved) plus everything that FK-blocks that deletion first (`notifications` → `broadcasts` → `tickets` → `attachments`, then orphaned `profiles`/`refresh_tokens`). User explicitly declined a backup/export first. Verified against `kinconnect_test` locally (542 employees + 1 non-master admin deleted cleanly, master admin untouched), then **run for real against production** the same day: 542 employees + 1 non-master admin (`K160`) deleted, 9 notifications/1 ticket/3 attachments/9 refresh tokens cleared, master admin `KN-01` untouched. Confirmed live via direct query: 0 employees, 1 admin (master only). Real employees still need to be re-imported (now with name + department) via the richer bulk-import feature above.
- **Fix**: Terms-and-Guidelines checkbox on the login page — `src/components/ui/checkbox.tsx`'s check icon was hardcoded `h-4 w-4` while `auth.tsx` overrides the box itself to a smaller `h-3.5 w-3.5` circle, so the tick visually overflowed its circle. Shrunk the icon to `h-3 w-3` (fits both the overridden circle here and the component's default `h-4 w-4` box, since this component currently has no other call sites).
- **Text change**: Footer copyright text "© 2026 Made by Yash Soni" → "© 2026 Powered by Yash Soni" across all three occurrences (`auth.tsx`, `admin.tsx`, `dashboard.tsx`).
- **Production deployment #2** (2026-07-25): Deployed everything from the 2026-07-24 session (see entries below). Order: (1) applied migration `0003` to the live Supabase DB (nullable `password_hash` on `employees`/`admins`) — run manually by the user via `DATABASE_URL=... npx tsx src/db/migrate.ts` from their own terminal, since the Claude Code auto-mode classifier blocks direct DB-migration commands against what it flags as a production connection string; (2) merged `fix/resolve-merge-conflicts` into `master` and pushed, which auto-triggered Render's backend rebuild; (3) rebuilt and redeployed the frontend to Cloudflare Workers — hit one tooling snag along the way: Nitro/Wrangler auto-generates a Workers `compatibility_date` from the local system clock, which was ahead of Cloudflare's own servers' real date and got rejected (`code: 10021`); fixed by deploying with an explicit past-dated override (`npx wrangler --cwd .output/ deploy --compatibility-date=2024-09-23`) instead of the plain `npx nitro deploy --prebuilt` wrapper. Verified live: a route that only exists in today's code (`/admin/employees/bulk-import`) returned 401 instead of 404, and the live login page visibly showed the new circular pre-checked T&C checkbox and password eye-icon. Also ran `wipe-activity-data.ts` (Required Update item 3) against production for real this time, after the user tested it thoroughly locally first — see "Required Update" section for exact counts. Item 4's forced 7→1 admin consolidation script was **not** run live — the client's own local testing of true multi-department assignment (Additional Changes below) superseded that plan; live `KN-02`–`KN-08` were left alone.
